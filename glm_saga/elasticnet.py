...
def elastic_loss(linear, X, y, lam, alpha, family='multinomial', sample_weight=None): 
    ...
    if family == 'multinomial': 
        ...
        if sample_weight is None:
            #Tuomas: add over different outputs instead of mean
            l = 0.5*F.mse_loss(linear(X),y,reduction='mean')*(y.shape[1])
        else: 
            l = 0.5*F.mse_loss(linear(X),y,reduction='none')
            l = (l*(sample_weight.unsqueeze(1))).mean()*(y.shape[1])
    else: 
...
def elastic_loss_and_acc(linear, X, y, lam, alpha, family='multinomial'): 
    ...
    if family == 'multinomial': 
        ...
        #Tuomas: add over different outputs instead of mean
        l = 0.5*F.mse_loss(outputs, y, reduction='mean')*(y.shape[1])
        ...
        for t in tqdm(range(nepochs)): 
            ...
            for batch in loader: 
                ...
                if family == 'multinomial': 
                    ...
                    if w is None: 
                        #Tuomas: add over different outputs instead of mean
                        loss = 0.5*F.mse_loss(out,y.to(weight.device), reduction='mean')*(y.shape[1])
                    else: 
                        loss = 0.5*F.mse_loss(out,y.to(weight.device), reduction='none')
                        loss = (loss*(w.unsqueeze(1))).mean()*(y.shape[1])
                    target = y
